...
def extract(url, table_attribs):
    ...
    for row in rows:
        ...
        if len(col) > 2:  # Ensure that the row has the correct number of columns
            ...
            name = col[1].get_text().strip()  # Using get_text to get the content of the cell directly
            mc_usd_billion = col[2].get_text().strip()  # Remove last character (e.g., '\n')
            mc_usd_billion = float(mc_usd_billion.replace(',', ''))  # Convert to float
